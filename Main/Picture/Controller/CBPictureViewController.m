...
@interface CBPictureViewController ()<UIScrollViewDelegate,JKNewsChannelViewDelegate,JKNewsChannelControllerDelegate>
...
- (NSMutableArray *)recommendChannels{
    if (!_recommendChannels) {
        ...
        NSData * data = [userDefault objectForKey:@"pictureBottomArr"];
...
- (NSMutableArray *)myChannels{
    if (_myChannels == nil) {
        ...
        NSUserDefaults * userDefault = [NSUserDefaults standardUserDefaults];
        NSData * data = [userDefault objectForKey:@"pictureTopArr"];
        NSArray *array = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
        for (int i = 0; i < array.count; i++) {
            JKType *type = [JKType new];
            type.title = array[i][@"name"];
...
- (void)setChildViewControllers{
    for (int i = 0; i < self.myChannels.count; i++){
        CBPictureListController * pictureList = [CBPictureListController new];
        JKType * type = self.myChannels[i];
        pictureList.pictureID = type.type;
        [self addChildViewController:pictureList];
        [self.childControllers addObject:pictureList];
    }
...
- (void)JKNewsChannelController:(JKNewsChannelController *)controller didAddItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CBPictureListController * content = [CBPictureListController new];
