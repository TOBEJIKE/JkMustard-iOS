...
@interface CBVideoViewController ()<UIScrollViewDelegate,JKNewsChannelViewDelegate,JKNewsChannelControllerDelegate>
...
- (NSMutableArray *)myChannels{
    if (_myChannels == nil) {
        ...
        NSUserDefaults * userDefault = [NSUserDefaults standardUserDefaults];
        NSData * data = [userDefault objectForKey:@"videoTopArr"];
        NSArray *array = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
        for (int i = 0; i < array.count; i++) {
            JKType *type = [JKType new];
            type.title = array[i][@"name"];
...
- (void)setChildViewControllers{
    for (int i = 0; i < self.myChannels.count; i++){
        CBVideoListController * videoList = [CBVideoListController new];
        JKType * type = self.myChannels[i];
        videoList.videoID = type.type;
        [self addChildViewController:videoList];
        [self.childControllers addObject:videoList];
    }
...
- (void)JKNewsChannelController:(JKNewsChannelController *)controller didAddItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CBVideoListController * content = [CBVideoListController new];
